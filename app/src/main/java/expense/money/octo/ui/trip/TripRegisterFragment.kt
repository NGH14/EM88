...
open class TripRegisterFragment :
	...
	private fun isValidForm(): Boolean {
		...
		if (name?.none { !it.isWhitespace() } == null) errors.add("Name cannot be empty")
		if (destination?.none { !it.isWhitespace() } == null) errors.add("Destination cannot be emtpy")
		if (dateOfTrip?.none { !it.isWhitespace() } == null) errors.add("Date of Trip cannot be empty")
